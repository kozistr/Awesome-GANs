...
def main():
    ...
    with tf.Session(config=config) as s:
        ...
        dataset_iter_shoes = DataIterator(img_shoes, None, paras['batch_size'])
        dataset_iter_bags = DataIterator(img_bags, None, paras['batch_size'])

        # sample shoes & bags
        sample_shoes = dataset.batch_shoes[:model.sample_num].astype(np.float32)
        sample_bags = dataset.batch_bags[:model.sample_num].astype(np.float32)

        d_overpowered = False  # G loss > D loss * 2
        for epoch in range(paras['epoch']):
            for img_shoes, _, img_bags, _ in zip(dataset_iter_shoes.iterate(), dataset_iter_bags.iterate()):
                # update D network
                if not d_overpowered:
                    s.run(model.d_op, feed_dict={model.shoes_x: img_shoes,
                                                 model.bags_x: img_bags})

                # update G network
                s.run(model.g_op, feed_dict={model.shoes_x: img_shoes,
                                             model.bags_x: img_bags})
                ...
                if step % paras['logging_interval'] == 0:
                    ...
                    iu.save_images(s2b_samples, size=[sample_image_height, sample_image_width], image_path=sample_dir)
                    iu.save_images(b2s_samples, size=[sample_image_height, sample_image_width], image_path=sample_dir)
