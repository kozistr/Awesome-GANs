...
class CGAN:
    ...
    def __init__(self, s, batch_size=64,
                 ...
                 n_hidden_layer_1=128, n_hidden_layer_2=256, epsilon=1e-9):
        self.s = s
    ...
    def discriminator(self, x, y, reuse=None):  # simple neural networks
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            net = tf.reshape(net, [-1, self.maxout_unit, self.n_hl_2])
    ...
    def build_cgan(self):
        ...
        self.W = {
            ...
            'd_h2': tf.get_variable('d_h2', shape=[self.n_hl_1, self.maxout_unit * self.n_hl_2],
                                    initializer=tf.contrib.layers.variance_scaling_initializer()),
            'd_h_out': tf.get_variable('d_h_out', shape=[self.n_hl_2, 1],
                                       initializer=tf.contrib.layers.variance_scaling_initializer()),
            # weights for generator
            'g_h1': tf.get_variable('g_h1', shape=[self.z_dim + self.n_classes, self.n_hl_2],
                                    initializer=tf.contrib.layers.variance_scaling_initializer()),
        ...
        self.b = {
            ...
            'd_b2': tf.Variable(tf.zeros([self.maxout_unit * self.n_hl_2])),
