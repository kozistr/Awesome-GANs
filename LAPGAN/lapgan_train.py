...
def main():
    ...
    with tf.Session(config=config) as s:
        ...
        model = lapgan.LAPGAN(s, batch_size=train_global_step['batch_size'])
        ...
        ds_iter = DataIterator(dataset.train_images, dataset.train_labels, train_global_step['batch_size'])

        global_step = 0
        cont = int(global_step / 750)
        for epoch in range(cont, cont + train_global_step['epoch']):
            for batch_images, batch_labels in ds_iter.iterate():
                ...
                for i in range(3):
                    z.append(np.random.uniform(-1., 1.,
                                               [train_global_step['batch_size'], model.z_noises[i]]).astype(np.float32))
                ...
                if global_step % train_global_step['logging_interval'] == 0:
                    ...
                    # Summary saver
                    model.writer.add_summary(summary, global_step=global_step)  # time saving
                    ...
                    # Model save
                    model.saver.save(s, results['model'], global_step=global_step)  # time saving
